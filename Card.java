...
public class Card {
   private byte[] Stats = new byte[15];//0=manacost
 ...
 //7: gain 1+/1+
   ...
   private final int MANACOST = 0, ATK =1, HEALTH = 2, BATTLECRY = 3, DEATHRATTLE = 4, STARTOFTURN = 5, ENDOFTURN = 6,
   	TAUNT = 7, PLAYER = 8, BATTLECRYDAMAGE = 9, HEALING = 10, TURNPLAYED = 11, HASMOVED = 12, HASATTACKED = 13, SPELL = 14;
   private String NAME;
 //Scanner input = new Scanner(System.in);

   public Card(String N,byte M, byte A, byte H, byte B, byte D, byte S, byte E, byte T, byte P,byte BD, byte HE, byte TP, byte HM, byte HA, byte SP)
   ...
   //The card and things that are required, in this case all of the values of the card if it has...
   {
      ...
      Stats[HASMOVED] = HM; //makes it so the piece can move only once per turn
      Stats[HASATTACKED] = HA; // makes it so the piece can attack once per turn
      Stats[SPELL] = SP; //says if a card is a spell or not, if the card is a spell it goes straight to the discard pile
   ...
   //finding indicies of all commas
    ...
    //NAME = parts[0];//x.substring(0,CommaLoc[0]);
      ...
      Stats[HASMOVED] = Byte.parseByte(parts[HASMOVED]);
      Stats[HASATTACKED] = Byte.parseByte(parts[HASATTACKED]);
      Stats[SPELL] = Byte.parseByte(parts[SPELL]);
...
//these methods return and set their corresponding values
   ...
   public byte getHasMoved()
   {
      ...
      return Stats[HASMOVED];   
   }
   
   public void setHasMoved(byte nHasmoved)
   {
   
      Stats[HASMOVED] = nHasmoved;
   
   }
   
   
   public byte getHasAttacked()
   {
   
      return Stats[HASATTACKED];   
   }
   
   public void setHasAttacked(byte nHasattacked)
   {
   
      Stats[HASATTACKED] = nHasattacked;
