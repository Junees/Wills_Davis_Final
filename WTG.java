...
// runs the game see the word document for rules
   ...
   //pre: no condition just runs the most basic commands the game needs
...
//pre: requires the playerNumber 
//post: sets all the cards on the board to not having attacked or moved at the start of a turn
//allowing the cards to attack at the start of a turn after having the values change after attacking/moving

   public static void boardSet(byte playerNum)
   {
      ...
      if(playerNum == 0)
      {
         for (int r = 0; r < sm.numRows(); r++) {
            for (int c = 0; c < sm.numColumns(); c++) {
               Card temp = sm.get(r, c);
               if (temp != null && temp.getHasMoved() == 1 && playerNum == 0)
                  temp.setHasMoved((byte) 0);
               if(temp != null && temp.getHasAttacked() == 1 && playerNum == 0)
                  temp.setHasAttacked((byte) 0);
               if(temp != null && temp.getTurnPlayed() == 1 && playerNum == 0)
                  temp.setTurnPlayed((byte) 0);
         ...
         for (int r = 0; r < sm.numRows(); r++) {
            for (int c = 0; c < sm.numColumns(); c++) {
               Card temp = sm.get(r, c);
               if (temp != null && temp.getHasMoved() == 1 && playerNum == 1)
                  temp.setHasMoved((byte) 0);
               if(temp != null && temp.getHasAttacked() == 1 && playerNum == 1)
                  temp.setHasAttacked((byte) 0);
               if(temp != null && temp.getTurnPlayed() == 1 && playerNum == 1)
                  temp.setTurnPlayed((byte) 0);
   ...
   
   {
      ...
      System.out.println("5: move your cards");
...
//post: conducts battle between two cards, the location you are sending it within dealBattleDamage and checks if it has summoning sickness
   ...
   //pre: requires the playerNum
   //post: run in the mainPhase method, telling what happens when a card is moved
   
   public static void moveStep(byte playerNum)
   {
   
      showToScreen();
   
      System.out.println("choose the card you want to move");
      System.out.println("x coord");
      byte xCoord2 = (byte) (input.nextInt());
      System.out.println("y coord");
      byte yCoord2 = (byte) (input.nextInt());
      ...
      if(sm.get(xCoord2,yCoord2) != null && sm.get(xCoord2,yCoord2).getHasMoved() == 1)
      {
      
         System.out.println("the commander cannot move on the first turn, or the piece has already moved");
         return;
      ...
      move(xCoord2, yCoord2, playerNum);
      
      if(sm.get(xCoord2,yCoord2) != null)
      {
         sm.get(xCoord2,yCoord2).setHasMoved((byte) 0);
      }
   ...
   public static void castSpell(byte playerNum, ArrayList<Card> hand, ArrayList<Card> discard)
   {
   
      System.out.println("spells in hand");
   
      for (int r = 0; r < hand.size(); r++) 
      {
      
         {
            if(hand.get(r).getSpell() == 1)
      ...
      System.out.println("choose the spell you want to cast");
   ...
   public static void move(byte xCoord, byte yCoord,byte playerNum)
   {
      ...
      Card card = sm.get(xCoord, yCoord);
      
      sm.remove(xCoord, yCoord);
   
      
      sm.add(xCoord2, yCoord2, card);
...
//post: removes the card from that player's hand, adds it to the field and checks for battlecry effects
   ...
   //post: adds the amount of the card from the player's mana

   public static byte addMana(byte playerNum, byte manaCost) {
   ...
   //pre: no condition
   //post: shows the stats of all the cards on the field
...
//pre: no condition
//post: same thing as draw card but from the discard pile
   ...
   public static void drawCard(byte pNum) {
   
      if(!deck0.equals(0) || !deck1.equals(0))
      {
         if (pNum == 0) {
...
//post: subtracts the health from the card and removes it from the board. Similar to dealing battle damage but this is related to spells and battlecry effects i.e. Deal 3 Damage
   ...
   public static void dealDamage(byte x, byte y)
   {
...
//shows all the cards on the board
...
//post: checks if everything is valid, and if so allows the player to move
   ...
   public static String validInputToMove(byte mr, byte mc, byte cr, byte cc) {
      ...
      if (mr < 0 || cr >= sm.numRows() || mc < 0 || cc >= sm.numColumns()) {
         ...
         return "the new location has bad coordinates";
      ...
      if(sm.get(mr,mc) != null)
      {
      
      
         return "cannot move on top of another piece";
      
      }
   
      Card curr = sm.get(mr, mc);
      Card newLoc = sm.get(cr, cc);
   
      byte dr = (byte) Math.abs(mr - cr);
      byte dc = (byte) Math.abs(mc - cc);
